...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 新增：检测任意选中文本的命令
    const checkSelectedTextCommand = vscode.commands.registerCommand(
        'xguard-commit-guard.checkSelectedText',
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showWarningMessage('请先打开一个文件');
            ...
            try {
                ...
                if (api && api.getAPI) {
                    ...
                    if (git && git.repositories && git.repositories.length > 0) {
                        // 监听提交后事件（用于日志记录）
                        git.onDidCommit?.(async (commit: { message: string }) => {
                            // 这里可以添加提交后的日志记录
...
// 新增：增强Git提交命令拦截
function enhanceGitCommitCommands(context: vscode.ExtensionContext, commitProvider: CommitMessageProvider) {
    // 拦截标准的Git提交命令
    ...
    // 重写git.commit命令
    const enhancedGitCommit = vscode.commands.registerCommand('git.commit', async (...args: any[]) => {
        // 提取commit message
        let commitMessage = '';
    ...
    const enhancedGitCommitStaged = vscode.commands.registerCommand('git.commitStaged', async (...args: any[]) => {
        let commitMessage = '';
        if (args.length > 0 && typeof args[0] === 'string') {
            commitMessage = args[0];
        }
    ...
    // 重写git.commitAll命令
    const enhancedGitCommitAll = vscode.commands.registerCommand('git.commitAll', async (...args: any[]) => {
        let commitMessage = '';
        if (args.length > 0 && typeof args[0] === 'string') {
            commitMessage = args[0];
        }
...
async function checkCommitMessageSafety(commitMessage: string, commitProvider: CommitMessageProvider): Promise<boolean> {
    try {
        // 调用现有的检测方法
