...
def install_vsce():
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if result.returncode != 0:
                if result.stderr and "refused to install" in result.stderr.lower():
                    print("💡 提示：如果遇到权限问题，请以管理员身份运行此脚本")
        ...
        print(f"❌ 未找到npm命令，请确保Node.js已正确安装: {str(e)}")
        return False
    except Exception as e:
...
def choose_install_method():
    ...
    if choice in ['1', '3']:
        ...
        if not compile_extension():
            success = False
        # 添加开发模式安装
        elif not install_vscode_dev_extension():
            success = False
